...
game_over_scaled = pygame.transform.scale(game_over_image, (900, 450))
...
# Game over flag
game_over = False
enemies_eliminated = 0  # Track the number of eliminated enemies

# Function to reset enemy position
def reset_enemy():
    global enemy_rect1, current_enemy_surf
    enemy_rect1 = enemy_surf1.get_rect(midbottom=(random.randint(400, 800), ground_rect.top + 10))
    current_enemy_surf = enemy_surf1
...
while True:
    ...
    if not game_over:
        # enemy animation
        if enemies_eliminated < 150:  # Only animate the enemy if less than 150 enemies have been eliminated
        ...
        if enemy_hit_Counter == 5:
            ...
            enemy_hit_Counter = 0
            if enemies_eliminated < 150:
                reset_enemy()
            else:
                enemy_rect1.topleft = (-100, -100)  # Move the enemy off-screen
        ...
        if enemies_eliminated < 150:  # Only draw the enemy if less than 150 enemies have been eliminated
